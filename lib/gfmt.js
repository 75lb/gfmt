...
function GfmTable(input){
    ...
    if (Array.isArray(input)){
        ...
        this.columns = fieldNames.map(function(fieldName){
            var col = {};
            col.field = fieldName;
...
GfmTable.prototype.createRow = function(row){
    return this.columns.reduce(function(prev, column){
        return prev + "| " + s.padRight(typeof row[column.field] === "undefined" ? "" : row[column.field], column.width) + " ";
    }, "") + "|\n";
};
...
function getUniqueFieldNames(data){
    return data.reduce(function(prev, row){
        Object.keys(row).forEach(function(field){
            if (!a.contains(prev, field)) prev.push(field);
        });
        return prev;
    }, []);
}
